...
<head>
    ...
    <script>
        ...
        $.ajax({
            type:"get",
            url:"/admin.php/Home/AdminUser/MenuLists",
            dataType:"json",
    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h2><i class="fa fa-indent red"></i><strong>修改职位信息</strong></h2>
            </div>
            <div class="panel-body">
                <form action="/admin.php/Home/AdminUser/EditPosition" method="post" class="form-horizontal">

                    <div class="form-group">
                        <label class="col-md-3 control-label" for="PositionName">职位名称</label>
                        <div class="col-sm-2">
                            <?php $_SESSION['code']=mt_rand(1,1000);?>
                            <input type="hidden" name="scode" value="<?php echo $_SESSION['code']?>" />
                            <input type="hidden" name="r_id" value="<?php echo ($data2["r_id"]); ?>" />
                            <input type="text" id="PositionName" name="PositionName" class="form-control" value="<?php echo ($data2["role_name"]); ?>">
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-md-3 control-label">职位权限</label>
                        <div class="col-md-9">

                            <?php if(is_array($data)): foreach($data as $key=>$v): ?><div class="checkbox">
                                    <label>
                                        <input type="checkbox" <?php foreach($data2['child'] as $key=>$val){if($val['jurisdiction_id']==$v['j_id']){ echo "checked";}}?> name="PositionArr[]" value="<?php echo ($v["j_id"]); ?>"> <?php echo ($v["jurisdiction_name"]); ?>
                                    </label>
...
<script>
    function logout() {
        ...
        mizhu.confirm('温馨提醒', '确定要退出吗？', function() {
            $.get("/admin.php/Home/AdminUser/LogOut",
                    function(data,status){
                        if(status=='success'){
                            location.href = '/admin.php/Home/AdminUser/index';
                        }else{
