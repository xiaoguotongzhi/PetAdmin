        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="form-group" style="margin: 15px 0px;">
                    <div class="col-md-3">
                        <form action="/admin.php/Home/Journalism/JournalismSearchWord" method="get" onsubmit="return JSubSearch()">
                            <div class="input-group">


                                    <?php if($word == null): ?><input type="text" id="word" name="word" class="form-control" placeholder="请输入趣闻ID或关键字"><span class="input-group-btn">
                ...
                <div class="panel-body">
                    <table class="table table-striped table-bordered bootstrap-datatable datatable">
                        ...
                        <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                                ...
                                <td><?php echo ($vo["editer"]); ?></td>
                                <td>
                                    <?php
 if($vo['tags']==null){ echo "<span class='label label-default'>暂无</span>"; }else{ foreach(explode(",",$vo['tags']) as $name){ echo "<span class='label label-success'>$name</span>&nbsp;&nbsp;"; } } ?>
                                </td>
                                <td>
                                    <?php
 if(strlen($vo['froms'])>=20){ echo mb_substr($vo['froms'],0,20,'utf-8').'...'; }else{ echo $vo['froms']; } ?>
                                </td>
                                <td>
                                    <?php if($vo["introduction"] == 1): ?>单图
                                        <?php elseif($vo["introduction"] == 2): ?>多图
                                        <?php elseif($vo["introduction"] == 3): ?>视频
                                        <?php else: ?>无图<?php endif; ?>
                                </td>
                                <td><?php echo date('Y-m-d H:i:s',$vo['create_time']);?></td>
                                <td>
                                    <a class="btn btn-info" href="/admin.php/Home/Journalism/EditJournalismIdInfo/id/<?php echo ($vo["jid"]); ?>" title="修改">
                                        <i class="fa fa-edit "></i>
                                    </a>
                                    <a class="btn btn-danger" href="javascript:void(0);" title="删除" onclick="JournalismDel(<?php echo ($vo["jid"]); ?>);">
                                        <i class="fa fa-trash-o "></i>
...
<script>
    ...
    function JournalismDel(JID) {
        ...
        mizhu.confirm('温馨提醒', '确定要删除吗？', function() {
            $.get("/admin.php/Home/Journalism/JournalismDel/id/" + JID,
