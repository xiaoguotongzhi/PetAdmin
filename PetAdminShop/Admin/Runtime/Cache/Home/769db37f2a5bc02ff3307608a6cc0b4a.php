        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="form-group" style="margin: 15px 0px;">
                    <div class="col-md-3">
                        <form action="/admin.php/Home/Orders/OrdersSearchWord" method="get" onsubmit="return SubSearch()">
                            <div class="input-group">


                                <?php if($word == null): ?><input type="text" id="word" name="word" class="form-control" placeholder="请输入订单号查询"><span class="input-group-btn">
                ...
                <div class="panel-body">
                    <table class="table table-striped table-bordered bootstrap-datatable datatable">
                        ...
                        <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                                ...
                                <td>
                                    <?php if($vo["status"] == 1): ?>未付款
                                        <?php elseif($vo["status"] == 2): ?>已付款
                                        <?php elseif($vo["status"] == 3): ?>已发货
                                        <?php elseif($vo["status"] == 4): ?>已收件
                                        <?php elseif($vo["status"] == 9): ?>问题件
                                        <?php else: ?> 未识别<?php endif; ?>
                                </td>
                                <td>
                                    <?php if($vo["admin_operation"] == 1): ?>未查看
                                        <?php elseif($vo["admin_operation"] == 2): ?>已查看
                                        <?php elseif($vo["admin_operation"] == 3): ?>处理完成
                                        <?php else: ?> 未识别<?php endif; ?>
                                </td>
                                <td>
                                    <a class="btn btn-info" href="/admin.php/Home/Orders/OrdersIdInfo/id/<?php echo ($vo["orders_id"]); ?>" title="详情">
                                        <i class="fa fa-edit "></i>
                                    </a>
                                    <a class="btn btn-danger" href="javascript:void(0);" title="删除" onclick="OrdersDel(<?php echo ($vo["orders_id"]); ?>);">
                                        <i class="fa fa-trash-o "></i>
...
<script>
    ...
    function OrdersDel(Id) {
        mizhu.confirm('温馨提醒', '确定要删除吗？', function() {
            $.get(
                    "/admin.php/Home/Orders/OrdersDel/id/"+ID,
