...
<head>
    ...
    <script>
        ...
        $.ajax({
            type:"get",
            url:"/admin.php/Home/Homepage/MenuLists",
            dataType:"json",
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
                ...
                <li><i class="fa fa-trophy"></i>首页管理</li>
                    ...
                    <table class="table table-striped table-bordered bootstrap-datatable datatable">
                        ...
                        <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                            <td>
                                ...
                                <input type="text" id="<?php echo ($vo["id"]); ?>" name="banner_order" class="form-control" value="<?php echo ($vo["banner_order"]); ?>" onblur="OrderSave(<?php echo ($vo["banner_order"]); ?>,<?php echo ($vo["id"]); ?>,$(this).attr('id'));" size="3">

                            </td>
                            <td><img class="img-thumbnail" src="<?php echo ($vo["img"]); ?>" alt="Sample Image" width="80px" height="80px;"></td>
                            <td>
                                <?php
 if(strlen($vo['html_url'])>=20){ echo mb_substr($vo['html_url'],0,20,'utf-8').'...'; }else{ echo $vo['html_url']; } ?>
                            </td>
                            <td><?php echo date('Y-m-d H:i:s',$vo['create_time']);?></td>
                            <td>
                                <?php if($vo["status"] == 1): ?><span class="label label-success">正常</span>
                                    <?php else: ?><span class="label label-danger">未显示</span><?php endif; ?>

                            </td>
                            <td>
                                <a class="btn btn-info" href="/admin.php/Home/Homepage/EditBannerIdInfo/id/<?php echo ($vo["id"]); ?>" title="修改">
                                    <i class="fa fa-edit "></i>
                                </a>
                                <a class="btn btn-danger" href="javascript:void(0);" title="删除" onclick="BannerDel(<?php echo ($vo["id"]); ?>);">
                                    <i class="fa fa-trash-o "></i>
    ...
    <script>
        function OrderSave(OldOrder,ID,AttrId) {
           var banner_order = $("#"+AttrId).val();
            if(OldOrder!=banner_order){
                $.get("/admin.php/Home/Homepage/AdminOrderSave/banner_order/"+banner_order+"/id/"+ID,
        ...
        function BannerDel(ID) {
            ...
            mizhu.confirm('温馨提醒', '确定要删除吗？', function() {
                $.get("/admin.php/Home/Homepage/BannerIdDel/id/" + ID,
                        function (data, status) {
                            if (status == 'success') {
...
<script>
    function logout() {
        ...
        mizhu.confirm('温馨提醒', '确定要退出吗？', function() {
            $.get("/admin.php/Home/Homepage/LogOut",
                    function(data,status){
                        if(status=='success'){
                            location.href = '/admin.php/Home/Homepage/index';
                        }else{
