...
<head>
    ...
    <script>
        ...
        $.ajax({
            type:"get",
            url:"/admin.php/Home/User/MenuLists",
            dataType:"json",
                ...
                <div class="panel-body">
                    <table class="table table-striped table-bordered bootstrap-datatable datatable">
                        ...
                        <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                                ...
                                <td>
                                    <?php if($vo["job_status"] == 1): ?>医生
                                        <?php elseif($vo["job_status"] == 2): ?>救援队
                                        <?php elseif($vo["job_status"] == 3): ?>美容师
                                        <?php elseif($vo["job_status"] == 4): ?>训犬师
                                        <?php else: ?> 其他<?php endif; ?></td>
                                <td><img src="<?php echo ($vo["front_photo"]); ?>" width="200px;" height="150px;"></td>
                                <td><img src="<?php echo ($vo["job_card"]); ?>" width="200px;" height="150px;"></td>
                                <td>
                                    <?php if($vo["status"] == 0): ?><span class="label label-warning">待审核</span>
                                        <?php elseif($vo["status"] == 1): ?><span class="label label-success">通过</span>
                                        <?php elseif($vo["status"] == 2): ?><span class="label label-default">未通过</span>
                                        <?php else: ?> <span class="label label-danger">高风险内容</span><?php endif; ?>
                                </td>
                                <td><?php echo date('Y-m-d H:i:s',$vo['create_time']);?></td>
                                <td>
                                    <?php if($vo["status"] == 0): ?><a class="btn btn-success" href="javascript:void(0);" title="通过" onclick="CardOk(<?php echo ($vo["card_id"]); ?>)"><i class="fa fa-thumbs-o-up "></i></a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-info" href="javascript:void(0);" title="拒绝" onclick="CardNo(<?php echo ($vo["card_id"]); ?>)"><i class="fa fa-thumbs-o-down "></i></a>
                                        <?php else: ?> <a class="btn btn-danger" href="javascript:void(0);" title="删除" onclick="CardIdDel(<?php echo ($vo["card_id"]); ?>);">
                                            <i class="fa fa-trash-o "></i>
                                        </a><?php endif; ?>
                                </td>
...
<script>
    function CardOk(Id) {
        ...
        mizhu.confirm('温馨提醒', '确定要通过这条信息吗？', function() {
            $.get("/admin.php/Home/User/CardOk/id/" + Id,
    ...
    function CardNo(Id) {
        mizhu.confirm('温馨提醒', '确定要拒绝这条信息吗？', function() {
            $.get("/admin.php/Home/User/CardNo/id/" + Id,
                    function (data, status) {
                        mizhu.toast(data.msg);
                        history.go(0);
    ...
    function CardIdDel(Id) {
        ...
        mizhu.confirm('温馨提醒', '确定要删除吗？', function() {
            $.get("/admin.php/Home/User/CardIdDel/id/" + Id,
                    function (data, status) {
                        if (status == 'success') {
                            mizhu.toast(data.msg);
                            history.go(0)
...
<script>
    function logout() {
        ...
        mizhu.confirm('温馨提醒', '确定要退出吗？', function() {
            $.get("/admin.php/Home/User/LogOut",
                    function(data,status){
                        if(status=='success'){
                            location.href = '/admin.php/Home/User/index';
                        }else{
