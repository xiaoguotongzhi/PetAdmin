 ...
 if(strlen($vo['content'])>=20){ echo mb_substr($vo['content'],0,20,'utf-8').'...'; }else{ echo $vo['content']; } ?>
                            ...
                            <td>
                                <?php if($vo["content_status"] == 1): ?><span class="label label-warning">通过</span>
                                    <?php else: ?><span class="label label-warning">待审核</span><?php endif; ?>
                            </td>
                            <td>
...
<script>
    function EBok(Id) {
        if(confirm("您确定要通过吗？")) {
            $.get("/admin.php/Home/Encyclopedias/EBok/id/" + Id,
    ...
    function EBno(Id,author_id) {
        if(confirm("您确定要拒绝吗？")) {
            $.get("/admin.php/Home/Encyclopedias/EBno/id/" + Id+"/author_id/"+author_id,
                    function (data, status) {
                        if (status == 'success') {
                            alert(data.msg);
                            history.go(0)
